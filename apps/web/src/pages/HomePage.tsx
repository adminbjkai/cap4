...
export function HomePage() {
  ...
  const phaseBucket = (phase?: string | null): LibraryFilter => {
    if (!phase || phase === "queued" || phase === "downloading" || phase === "probing" || phase === "processing" || phase === "uploading" || phase === "generating_thumbnail") {
      return "processing";
    }
  ...
  const filteredItems = libraryItems.filter((item) => (filterBy === "all" ? true : phaseBucket(item.processingPhase) === filterBy));
      ...
      <section>
        ...
        {loadingLibrary && libraryItems.length === 0 ? (
          ...
          <div className="panel-subtle border-dashed flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="mt-1 max-w-xs text-sm text-muted">Record or upload your first clip to start building your library.</p>
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 animate-in fade-in slide-in-from-bottom-2 duration-500">
              {visibleItems.map((item, index) => (
                <div
                  ...
                  style={{ animationDelay: `${index * 50}ms` }}
                >
                  <Link to={`/video/${item.videoId}`} className="relative aspect-video w-full overflow-hidden bg-surface-muted">
                    {item.thumbnailKey ? (
                  ...
                  <div className="relative z-10 -mt-10 p-4">
                    <div className="flex items-start justify-between gap-2">
                      <h3 className="truncate text-sm font-bold leading-none text-white drop-shadow-sm">{item.displayTitle}</h3>
                      <span
