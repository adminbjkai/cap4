  ...
  if (!status && loading) {
    return (
      <div className="space-y-4">
        ...
        <ProviderStatusPanel data={providerStatus} loading={loading} errorMessage={providerStatusError} compact />
      </div>
    );
  }
    ...
    <div className="space-y-4">
      <section className="workspace-card">
        ...
        <ol className="mb-4 grid gap-2 sm:grid-cols-4 xl:grid-cols-7">
          {steps.map((step) => {
            const isComplete = rank > step.rank || (step.key === "complete" && phase === "complete");
            ...
            return (
              ...
              >
                ...
                {step.label}
                {isActive ? <p className="mt-1 text-[10px] font-semibold uppercase tracking-wide">Current</p> : null}
                {isUpcoming ? <p className="mt-1 text-[10px] uppercase tracking-wide opacity-70">Pending</p> : null}
              </li>
            );
          })}
        </ol>
